document.addEventListener('DOMContentLoaded', function () {
    ...
    function displayMinionDetails(minion) {
        ...
        const headers = ['Tier'];
        const filteredProducts = minion.products.filter(product => product.item !== 'Diamond (Spreading)');
        ...
        updateGeneratedAmounts(1, minion); // Initialize with default value

        // Event listener for hours input
        hoursInput.addEventListener('input', function () {
            const hours = parseFloat(this.value) || 1; // Default to 1 if invalid
            updateGeneratedAmounts(hours, minion);
        });
    }

    function updateGeneratedAmounts(hours, minion) {
